...
class _TempConvUnit(nn.Module):
    def __init__(
        ...
        self.bn = nn.BatchNorm2d(
            out_channels, momentum=batch_norm_alpha, affine=True, eps=1e-5
        )
    ...
    def __init__(self, channel: int):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                ...
                m.weight.data = (
                    torch.from_numpy(np.concatenate((Lo_D, Hi_D), axis=0))
                    .unsqueeze(1)
                    .unsqueeze(1)
...
class TSSEFFNet(nn.Module):
    def __init__(
        ...
        self.avg_pool1 = nn.AdaptiveAvgPool2d((1, 61))
